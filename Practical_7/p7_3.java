...
class AccountHolder {
    ...
    void info() {
        ...
        System.out.println("Enter Your Net Monthly Income : "); inc = sc.nextDouble();
    ...
    void check() {
        if (age >= 21 && age <= 60) { System.out.println("Your Age is : " + age);
        } else if (inc >= 15000) {
...
// If you want user input, do it in the loop below for (AccountHolder p : personHolder) {
        for (AccountHolder p : personHolder) {
            p.info();

            p.info();
        p.check();
        if (p.age >= 21 && p.age <= 60 && p.inc >= 15000 && p.we
                >= 1 && p.czs == true) {
